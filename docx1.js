...
async function processQuestions(inputArray) {
  ...
  const processedArray = inputArray.map((item) => {
    ...
    const hasPassage = Object.keys(questionObj).some((key) => key.toLowerCase().includes('texte'));
...
function transformArray(inputArray) {
  ...
  const transformedArray = inputArray.map((item) => {
    ...
    return {
      ...
      instruction: innerKey.toLowerCase().includes('texte') ? PASSAGES[innerKey] : innerKey,
